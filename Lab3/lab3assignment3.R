...
data <-data.frame("x" = generatedVars, "sin" = sin(generatedVars))
...
#w1=10, b1=10, w2=10, b2=1, so we get 10+10+10+1=31 weights
set.seed(1234567890)
winit <- runif(31, -1, 1)
...
plot(train, cex=2, main="logistic/sigmoid (default) activation function h(x) = 1/(1+exp(-x))") 
...
#2 act.fct: a differentiable function that is used for smoothing the result of the cross product of the covariate
...
g=seq(-4,4, by=0.01)
plot(g, relu(g), type="l", col="green", main ="ReLU (green) vs softplus (blue)")
points(g, softplus(g), type="l", col="blue")
